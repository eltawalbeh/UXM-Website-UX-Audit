...
import { NEW_FINDING_DRAFT, buildAiReviewedFindingDraft, buildEvidenceCompletePayload, buildFindingDraft, nextFindingId, resolveFindingEditorSelection, utcIsoToDatetimeLocal } from './src/finding-editor.js';
...
const operatorCopy = () => locale === 'ar' ? { brand:'مساحة عمل تدقيق UXM', rail:'تنقل المشغّل', system:'نظام المشغّل', portfolio:'المحفظة', audits:'التدقيقات', templates:'القوالب', deliveries:'التسليمات', next:'المرحلة التالية', principle:'تشغيل قائم على الأدلة', boundary:'الذكاء يكتشف. البشر يراجعون. الدليل يضبط التسليم.' } : { brand:'UXM Audit workspace', rail:'Operator navigation', system:'Operator system', portfolio:'Portfolio', audits:'Audits', templates:'Templates', deliveries:'Deliveries', next:'Next slice', principle:'Evidence-led operation', boundary:'AI discovers. Humans validate. Evidence controls delivery.' };
const workspaceRail = () => { const copy=operatorCopy(); return `<aside class="workspace-rail" aria-label="${copy.rail}"><a class="workspace-rail__brand" href="/workspace.html" aria-label="${copy.brand}">UXM<span>Audit</span></a><div><p class="workspace-rail__label">${copy.system}</p><nav class="workspace-rail__nav" aria-label="${copy.rail}"><a class="workspace-rail__link" href="/operations.html">${copy.portfolio}</a><button type="button" class="workspace-rail__link is-current" data-view="overview">${copy.audits}</button><a class="workspace-rail__link" href="/templates.html">${copy.templates}</a><span class="workspace-rail__future" aria-disabled="true">${copy.deliveries}<small>${copy.next}</small></span></nav></div><div class="workspace-rail__footer"><strong>${copy.principle}</strong><span>${copy.boundary}</span></div></aside>`; };
...
function criteria() {
  ...
  const issues = entries.filter(([, status]) => status === 'issue').length;
  const unverified = entries.filter(([, status]) => status === 'not_verified').length;
  shell(`<section class="criteria-workbench"><aside class="criteria-workbench__sections"><p class="operator-overview__eyebrow">Assessment library</p><h2>Criteria review</h2><p class="muted">Assess a checkpoint, then create and defend the linked finding.</p><div class="criteria-workbench__filter"><strong>Current scope</strong><span>${entries.length} checkpoints</span></div><div class="criteria-workbench__filter"><strong>Needs attention</strong><span>${issues} issue · ${unverified} unverified</span></div><button class="button alt" data-view="findings">Open findings queue</button></aside><div class="criteria-workbench__table"><div class="criteria-workbench__heading"><div><p class="operator-overview__eyebrow">Human assessment</p><h1>Review evidence against the UXM library</h1></div><p class="muted">Only applicable Pass, Partial, and Issue states contribute to the score.</p></div><table class="criteria"><thead><tr><th>ID</th><th>Checkpoint</th><th>Assessment</th><th>Section score</th></tr></thead><tbody>${entries.map(([id,status])=>`<tr><td><b>${id}</b></td><td>${criterionTitle(id)}</td><td><select data-assessment="${id}">${['pass','partial','issue','not_applicable','not_verified'].map(s=>`<option value="${s}" ${s===status?'selected':''}>${s.replace('_',' ')}</option>`).join('')}</select></td><td>${formatSectionScore(sectionScoreFor(r, id))}</td></tr>`).join('')}</tbody></table></div><aside class="criteria-workbench__inspector"><p class="operator-overview__eyebrow">Assessment rule</p><h2>Keep the official score defensible</h2><p>Issue requires an internal rationale and should lead to a human-reviewed finding. Not applicable and Not verified stay outside the score denominator.</p><div class="notice">Pass = 1 · Partial = 0.5 · Issue = 0</div></aside></section>`);
}
...
function render(){({overview,scope:findingsWorkbench,firstPass:findingsWorkbench,criteria,findings:findingEditor,scorecard,readiness:findingsWorkbench,report})[view]();}
