...
function scorecard(){const r=deriveReport(audit,locale); shell(`<div class="eyebrow">Review before publish</div><h1>Scorecard & priority review</h1><p class="sub">Critical and High priorities remain visible regardless of numerical score.</p><div class="grid"><section class="card"><div class="score">${scoreLabel(r)}</div><h2>Needs targeted improvement</h2><p class="muted">${Array.isArray(audit.assessments) ? audit.assessments.length : Object.keys(audit.assessments).length} checkpoints in this audit scope</p>${sectionRows(r)}</section><section class="card"><h2>Fix now</h2>${r.roadmap.fixNow.length?r.roadmap.fixNow.map(f=>`<div class="finding"><span class="chip high">2 High</span><h3>${f.id} — ${f.title}</h3><p>${f.recommendation}</p></div>`).join(''):'<p class="muted">No Critical or High findings yet.</p>'}<button class="button" data-view="report">Open client report</button></section></div>`);}
function report(){const r=deriveReport(audit,locale); const ar=locale==='ar'; shell(`<article class="report"><div class="report-head"><div><div class="eyebrow">UXM / ${ar?'تدقيق تجربة المستخدم':'Website UX Audit'}</div><h1>${audit.client}</h1><p class="sub">${audit.website} · ${audit.url}</p></div><button class="button alt" data-view="overview">${ar?'مساحة العمل':'Back to workspace'}</button></div><section class="card"><div class="grid"><div><div class="score">${scoreWithScale(r)}</div><h2>${ar?'يحتاج إلى تحسينات مركزة':'Needs targeted improvement'}</h2><p class="muted">${ar?'تم تقييم نطاق أولي من المعايير الأساسية':'Prototype coverage: initial core criteria assessed'}</p></div><div><h2>${r.labels.roadmap}</h2>${[['fixNow',r.labels.fixNow],['fixNext',r.labels.fixNext],['enhanceLater',r.labels.enhanceLater]].map(([key,label])=>`<div class="row"><span>${label}</span><b>${r.roadmap[key].length}</b></div>`).join('')}</div></div></section><section class="roadmap"><div class="eyebrow">${ar?'الملاحظات':'Findings'}</div><h1>${r.labels.roadmap}</h1>${audit.findings.length?audit.findings.map(f=>`<article class="finding"><span class="chip ${severityClass(f.severity)}">${f.severity==='high'?(ar?'2 عالية':'2 High'):f.severity==='medium'?(ar?'3 متوسطة':'3 Medium'):(ar?'4 منخفضة':'4 Low')}</span><h3>${f.id} — ${reportText(f,'title')}</h3><div class="grid">${workspaceEvidence(f)}<div><b>${ar?'ما لاحظناه':'What we observed'}</b><p>${reportText(f,'observed')}</p><b>${ar?'لماذا يهم ذلك':'Why it matters'}</b><p>${reportText(f,'impact')}</p><b>${ar?'التوصية':'Recommendation'}</b><p>${reportText(f,'recommendation')}</p></div></div></article>`).join(''):`<div class="notice">${ar?'أضف ملاحظة من مساحة العمل لإظهارها هنا.':'Add a finding in the workspace to populate the report.'}</div>`}</section></article>`);}
function render(){({overview,scope:findingsWorkbench,firstPass:findingsWorkbench,criteria,findings:findingsWorkbench,scorecard,readiness:findingsWorkbench,report})[view]();}
