import { deriveSectionScores } from './section-scoring.js';

const labels = {
  en: { roadmap: 'Priority roadmap', fixNow: 'Fix now', fixNext: 'Fix next', enhanceLater: 'Enhance later' },
  ar: { roadmap: 'خارطة أولويات التحسين', fixNow: 'معالجة فورية', fixNext: 'التحسينات التالية', enhanceLater: 'تحسينات لاحقة' },
};
...
export function createSeedAudit() {
  return {
    id: 'audit_demo_001', client: 'Northstar Services', website: 'northstar.example', url: 'https://northstar.example/',
...
export function updateAssessment(audit, criterionId, status) {
  return { ...audit, assessments: { ...audit.assessments, [criterionId]: status } };
}

export function createFinding(audit, data) {
  const id = `UXM-${String(audit.findings.length + 1).padStart(3, '0')}`;
  return { ...audit, findings: [...audit.findings, { id, ...data }] };
}
...
export function deriveReport(audit, locale = 'en') {
  ...
  return {
    direction: locale === 'ar' ? 'rtl' : 'ltr', labels: labels[locale], findings: audit.findings, roadmap,
    sectionScores: deriveSectionScores(audit.assessments),
  };
}
