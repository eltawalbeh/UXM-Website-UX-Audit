...
let state = { clients: [], projects: [], audits: [], query: '', message: '', error: '' };
...
async function loadOperations(message = '') {
  try {
    const [clients, projects, audits] = await Promise.all([
      jsonRequest('/api/clients'), jsonRequest('/api/projects'), jsonRequest('/api/audits'),
    ]);
    state = { ...state, clients, projects, audits, message, error: '' };
  } catch (error) {
